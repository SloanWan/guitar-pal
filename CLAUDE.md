...
- The shared core should not import from a domain. Today it does in two places (`missLog` → `strum/editIntent`, `smallTalk` → `strum/suggest`), and `tab/` reaches into `strum/` for `withoutCapo`, `blankSpan`, `BLANK`/`Guidance` and `explainEditIntent`; these are known and belong to the triage work (#191), not to be re-tangled further.
