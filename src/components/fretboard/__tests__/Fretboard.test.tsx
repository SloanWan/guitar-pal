...
describe("Fretboard", () => {
	it("renders every slot on the neck as a dormant node, lit only where a mark exists", () => {
		const host = render(A_ROOTS, 0, 12);
		const slots = host.querySelectorAll("[data-emphasis]");
		expect(slots).toHaveLength(6 * 13);
		...
		expect(host.querySelectorAll('[data-emphasis="none"]')).toHaveLength(6 * 13 - 5);
	});
	...
	it("keeps the whole neck in the DOM behind a window and slides to it", () => {
		const host = render(A_ROOTS, 5, 9);
		expect(host.querySelectorAll("[data-emphasis]")).toHaveLength(6 * 13);
		const neck = host.querySelector(".fb-neck") as HTMLElement;
		expect(neck.style.transform).toBe("translateX(-219px)"); // 1 − 5 × 44
		const svg = host.querySelector("svg") as SVGElement;
		expect(svg.getAttribute("data-from-fret")).toBe("5");
		expect(svg.getAttribute("data-to-fret")).toBe("9");
	});

	it("extends the neck to reach a mark beyond the minimum length", () => {
		const host = render([{ string: 0, fret: 15, label: "G", emphasis: "root" }], 0, 12);
		expect(host.querySelectorAll("[data-emphasis]")).toHaveLength(6 * 16);
		expect(slot(host, 0, 15).getAttribute("data-emphasis")).toBe("root");
	});

	it("labels the strings low E to high e and numbers every fret", () => {
		const host = render([], 0, 12);
		const text = host.textContent ?? "";
		for (const name of ["E", "A", "D", "G", "B", "e"]) expect(text).toContain(name);
		for (let fret = 0; fret <= 12; fret++) expect(text).toContain(String(fret));
	});
	...
	it("knows nothing about scales or chords: only the mark model and the string names", () => {
		...
		expect(new Set(modules)).toEqual(new Set(["@/lib/chordVoicingToMidi", "@/lib/fretboard/types"]));
