...
import { createShare, newShareId, shareUrl, type SharedStrum } from "@/lib/sharedItems";
...
/**
 * The strumming machine. On /strum it opens the player's library; given
 * `shared`, it opens that snapshot instead (the /p/[id] route): the library and
 * every save path are hidden, the snapshot — its pattern, and the progression
 * that was open when it was shared — is what plays, and the one way to change
 * it is to import it, a copy into the viewer's own library.
 */
export default function StrumWorkspace({ shared }: { shared?: SharedStrum }) {
	...
	const [patternRestored, setPatternRestored] = useState(isShared);
	// A shared progression opens at its own tempo, else the pattern's.
		...
		() =>
			shared
				...
				: selectedPattern
					...
					: [],
		[progressions, selectedPattern, shared],
	);
	...
	useEffect(() => {
		// A share page is not where a confirmed proposal lands: leave the stash
		// for the player's own page to take.
		if (isShared) return;
	...
	const patternRestoredRef = useRef(isShared);
	useEffect(() => {
	...
	useEffect(() => {
		// A shared pattern is in nobody's library; it is not "deleted" for that.
		if (isShared || patternsLoading || !selectedPattern) return;
	...
	}, [customPatterns, patternsLoading, isShared]);
	...
	useEffect(() => {
		...
		if (isShared || !quiet || !openProgression || !selectedPattern) return;
	...
	async function handleShare() {
		...
		if (!user) {
			toast("Sign in to share a pattern.", {
				action: { label: "Sign in", onClick: () => router.push("/auth?redirect=/strum") },
			});
			return;
		}
	...
	async function handleImport() {
		if (!shared || patternsLoading || importing) return;
		setImporting(true);
		try {
			const pattern: StrumPattern = {
				...shared.pattern,
			...
			stop();
			if (user) {
				toast(`Saved "${pattern.name}" to your patterns.`);
			} else {
				toast(`Saved "${pattern.name}" to this browser.`, {
					description: "Sign in to keep it on your account.",
					action: { label: "Sign in", onClick: () => router.push("/auth?redirect=/strum") },
				});
			}
			router.push(`/strum?pattern=${pattern.id}`);
		} catch (e) {
			console.error(e);
		...
		<>
			<div className="md:h-[calc(100vh-3.5rem)] flex flex-col md:flex-row md:overflow-hidden bg-workspace">
				{/* Left sidebar — lg: static; below lg: slide-in overlay. A shared
				    snapshot has no library: the sidebar says what this is and offers
				    the import, and below lg that lives in a strip over the card
				    instead of behind a toggle. */}
				{shared && selectedPattern ? (
				...
				>
					...
					<div className={`flex max-h-full w-full max-w-160 flex-col ${shared ? "" : "pt-10 lg:pt-0"}`}>
						{shared && selectedPattern && (
						...
						{!patternRestored ? (
							...
							<PatternWorkspace
								...
								patternSync={
									!isShared && (openSyncState?.kind === "ask" || openSyncState?.kind === "detached")
										? openSyncState.kind
										: null
								}
								...
								// Pinning a shape writes to the account; not from a share page.
								onApplyChordShape={isShared ? undefined : handleApplyChordShape}
								userVoicings={userVoicings}
								onSaveVoicing={isShared ? undefined : saveUserVoicing}
								onShare={isShared ? undefined : handleShare}
								sharing={sharing}
								readOnly={isShared}
								...
								onProgressionBarChord={
									openProgression && !isShared ? handleProgressionBarChord : undefined
								}
								onEditPattern={
