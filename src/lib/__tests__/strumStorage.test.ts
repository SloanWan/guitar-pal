...
describe("importStrumPattern as a guest", () => {
	...
	it("stamps the pattern, lists it newest first, and files the progression", async () => {
		localStorage.setItem(
			STRUM_PATTERNS_STORAGE_KEY,
			JSON.stringify([{ id: "old", name: "Old", beats, createdAt: "2020-01-01T00:00:00.000Z" }]),
		);
		await importStrumPattern(noClient, null, pattern, progression);
		...
		expect(readStoredProgressions()).toEqual([progression]);
	});

	it("leaves the progressions alone when there is none to import", async () => {
		await importStrumPattern(noClient, null, pattern, null);
