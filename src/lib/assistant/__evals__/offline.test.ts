...
import { EVAL_CASES } from "@/lib/assistant/__evals__/cases";
import { gradeProposal, summarize } from "@/lib/assistant/__evals__/grade";
 ...
 * full answer for every case the app reads itself. Part of `npm test`. The
 * turn resolver is synchronous and has no way to reach the network, which is
 * the whole of the "no model was called" assertion now.
 *
...
describe("strum assistant eval set — offline", () => {
	...
	for (const c of deterministic) {
		it(`answers "${c.input}" without the model  (${c.id})`, () => {
			const outcome = resolveAssistantTurn({ text: c.input, index: INDEX });
	...
	it("reports how much of the set never costs a model call", () => {
		const results = [];
		for (const c of deterministic) {
			const outcome = resolveAssistantTurn({ text: c.input, index: INDEX });
