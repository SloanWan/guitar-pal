 ...
 * `6(32)1(32)`, `根3231323`, `R_32^132`, `5/4 2 1 3`. The editor's Pick box
 * grammar — digits, rests, holds, pinches and the root token — plus one
 * thing a sentence needs that a box does not: an alternating bass.
 *
...
/** A word that could be part of an order: digits, rests, holds, pinches (either width), a root, an alternation. */
const ORDER_WORD = /^(?:[0-6\-_^＿＾()（）根Rr]+|[456]\/[456])$/;
...
export function parsePickOrder(text: string): PickOrderParse {
	...
	if (stray !== undefined) {
		return { ok: false, error: `"${stray}" isn't a string number, a rest (0 or -), a hold (_ or ^) or a pinch.` };
	}
