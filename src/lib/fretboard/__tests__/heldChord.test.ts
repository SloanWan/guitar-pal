...
describe("heldChordView", () => {
	it("is the same chord on both sides without a capo, with the key's numeral when its notes are the key's", () => {
		...
		const shown = heldChordView({ root: "A", suffix: "m7", voicingId: null }, C_MAJOR, 0, [9, 0, 4, 7]);
	...
	it("sounds the shape moved up by the capo, and judges the numeral by what sounds", () => {
		...
		const shown = heldChordView({ root: "A", suffix: "m7", voicingId: null }, C_MAJOR, 3, [0, 3, 7, 10]);
		...
		expect(heldChordView({ root: "A", suffix: "m7", voicingId: null }, { root: "Eb", scale: "major" }, 3, [0, 3, 7, 10]).numeral).toBe("vi");
	});
	...
	it("withholds the numeral for a chord with a note outside the key, or before its notes are known", () => {
		...
		expect(heldChordView({ root: "A", suffix: "7", voicingId: null }, C_MAJOR, 0, [9, 1, 4, 7]).numeral).toBeNull();
		expect(heldChordView({ root: "A", suffix: "m7", voicingId: null }, C_MAJOR, 0, null).numeral).toBeNull();
		// A root outside the scale never gets one, whatever the notes.
		expect(heldChordView({ root: "Eb", suffix: "major", voicingId: null }, C_MAJOR, 0, [3, 7, 10]).numeral).toBeNull();
	});
});
